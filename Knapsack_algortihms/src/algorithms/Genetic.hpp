...
*/
class Genetic : public SolveStrategy {
private:
	/*///////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
	// Genetic algorithm setting.
	const unsigned int MAX_GENERATIONS = 100;	// Number of generations.
	const unsigned int CHROMOZOME_POPULATION = 200;	// How many chromozmes in population will be.
	const unsigned int TOURNAMENT_SIZE = 3;	// Tournament size in seleciton.
	const unsigned int ELITISM_SIZE = (int)(ceil(CHROMOZOME_POPULATION * 0.8));	// How many new population will be in new generation.
	const unsigned int CROSSOVER_CHANCE = 90;	// Probability of crossover. In %.
...
public:
	InstanceSolution solve(const InstanceProblem & problem) {
		...
		// Set number of items.
		this->n = problem.getThings().size();
		// Maximum of random for items.
		rand_max_n = (unsigned long)(rand_max_n << this->n);
		...
		auto weight_and_value = count_weight_and_value(problem.getThings(), best_combination);
		return InstanceSolution(best_combination, best_fitness, weight_and_value.second, 0);
	}
...
private:
	...
	void fitness(Chromozome & chromozome) {
		int fitness = 0;
		auto weight_and_value = count_weight_and_value(problem.getThings(), chromozome.items);
		if (weight_and_value.second > problem.getCapacity()) {
			// Penalty
			fitness = 0;
		} else {
			fitness = weight_and_value.first;
		}
